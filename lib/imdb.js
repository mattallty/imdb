        ...
        if(id.match(/nm\d+/i)) {
            ...
            request('http://www.imdb.com/name/' + id + '/', function (error, response, html) {
                if (!error && response.statusCode == 200) {
                    ...
                    // var actor = body.match(/<span class="itemprop" itemprop="jobTitle">([a-z0-9_ ]+)<\/span>/i)[1];
                    // console.log(actor)
                    // var producer = body.match(/<div class="inline" itemprop="description">([a-z0-9_ ]+)to/i)[1];
                    ...
                    $("div.filmo-row").each(function(i, element) {
                        if (element.attribs.id.split("-",2)[0] == "actor")
                            console.log(element.children[0].next.next.next.children[0].children[0].data);
                    });
